...
class MySqlConnect
{
	...
	// Dentro de MySqlConnect.php
	public function escape($texto)
	{
		// Si por alguna razón hay una conexión abierta, la cerramos para empezar de cero
		if ($this->link) {
			@$this->link->close();
		}

		$this->connect();
		$resultado = mysqli_real_escape_string($this->link, $texto);
		$this->link->close();
